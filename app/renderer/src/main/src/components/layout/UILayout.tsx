...
const UILayout: React.FC<UILayoutProp> = (props) => {
    ...
    /** yakit使用状态 */
    const [yakitStatus, setYakitStatus, getYakitStatus] = useGetState<YakitStatusType | undefined>()
    ...
    const [latestYaklang, setLatestYkalng] = useState<string>("")

    /**
     * 1、获取软件运行环境变量
     * 2、获取操作系统类型
     * 3、获取yaklang本地版本和最新版本
     */
    useLayoutEffect(() => {
        setLoading(true)
        ...
        ipcRenderer.invoke("fetch-latest-yaklang-version").then((data: string) => {
            setLatestYkalng(data)
        })
    }, [])

    const isYaklangUpdate = useMemo(() => {
